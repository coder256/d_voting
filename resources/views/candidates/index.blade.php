...
@section('content')
    <div class="main-card mb-3 card">
        <div class="card-body">
                        ...
                        <tr>
                            ...
                            <td>
                                <form action="{{ route('candidate.destroy', $candidate->id)}}" method="post">
                                    @csrf
                                    @method('DELETE')
                                    <a href="{{ route('candidate.show', $candidate->id) }}" class="btn btn-info"><i
                                                class="fa fa-eye"></i></a>
                                    @if (in_array(Auth::user()->role, array('admin')))
                                    <a href="{{ route('candidate.edit', $candidate->id) }}" class="btn btn-warning"><i
            ...
            @else
                <a href="{{ route('candidate.create') }}" class="mb-2 mr-2 btn btn-success">Create</a>
            @endif
        </div>
    </div>
@endsection
